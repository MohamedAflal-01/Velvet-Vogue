...
$stmt = $conn->prepare("SELECT p.*, c.name as category_name, pi.image_url FROM products p JOIN categories c ON p.category_id = c.id LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.is_primary = 1 WHERE p.is_new = 1 AND p.status = 'active' ORDER BY p.created_at DESC");
$stmt->execute();
...
<div class="container my-5">
    <div class="text-center mb-5">
        <h1 class="text-gold">New Arrivals</h1>
        <p class="text-muted">Fresh from the runway. Discover our latest styles designed for immediate updates to your wardrobe.</p>
    </div>
    
    <div class="row">
        <?php if(empty($products)): ?>
            <div class="col-12 text-center py-5">
                <h4 class="text-muted">No new arrivals at this moment.</h4>
            </div>
        <?php else: ?>
                ...
                <div class="col-md-3 mb-4">
                    <div class="card product-card h-100">
                        ...
                        <div class="card-body text-center">
                            <span class="badge bg-gold text-dark mb-2">NEW</span>
