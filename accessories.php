<?php
$page_title = "Accessories Collection";
require_once 'includes/header.php';

$db = new Database();
$conn = $db->getConnection();

// Fetch Accessories products (Category ID = 4)
$stmt = $conn->prepare("SELECT p.*, c.name as category_name, pi.image_url FROM products p JOIN categories c ON p.category_id = c.id LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.is_primary = 1 WHERE c.slug = 'accessories' AND p.status = 'active' ORDER BY p.created_at DESC");
$stmt->execute();
...
<div class="container my-5">
    <div class="text-center mb-5">
        <h1 class="text-gold">Accessories Collection</h1>
        <p class="text-muted">Elevate your look with luxury leather bags, fine jewelry, timepieces, and silk scarves.</p>
    </div>
    
    <div class="row">
        <?php if(empty($products)): ?>
            <div class="col-12 text-center py-5">
                <h4 class="text-muted">No accessories found in our catalogue.</h4>
            </div>
        <?php else: ?>
