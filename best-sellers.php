...
// Fetch Best Sellers (is_featured = 1 as dynamic mapping)
$stmt = $conn->prepare("SELECT p.*, c.name as category_name, pi.image_url FROM products p JOIN categories c ON p.category_id = c.id LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.is_primary = 1 WHERE p.is_featured = 1 AND p.status = 'active' ORDER BY p.price DESC");
$stmt->execute();
...
<div class="container my-5">
    <div class="text-center mb-5">
        <h1 class="text-gold">Best Sellers</h1>
        <p class="text-muted">Our client favourites. The most coveted pieces of the season, loved for their tailoring and design.</p>
    </div>
    
    <div class="row">
        <?php if(empty($products)): ?>
            <div class="col-12 text-center py-5">
                <h4 class="text-muted">No best sellers available yet.</h4>
            </div>
        <?php else: ?>
                ...
                <div class="col-md-3 mb-4">
                    <div class="card product-card h-100">
                        ...
                        <div class="card-body text-center">
                            <span class="badge bg-dark text-gold mb-2"><i class="fas fa-star me-1"></i> POPULAR</span>
