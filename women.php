...
$stmt = $conn->prepare("SELECT p.*, c.name as category_name, pi.image_url FROM products p JOIN categories c ON p.category_id = c.id LEFT JOIN product_images pi ON pi.product_id = p.id AND pi.is_primary = 1 WHERE p.gender = 'Women' AND p.status = 'active' ORDER BY p.created_at DESC");
$stmt->execute();
...
<div class="container my-5">
    <div class="text-center mb-5">
        <h1 class="text-gold">Women's Collection</h1>
        <p class="text-muted">Unveil elegant evening gowns, chic business formal attire, and everyday luxury coordinates.</p>
    </div>
